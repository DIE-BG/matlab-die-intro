classdef polynomial
    ...
    methods
        function  self = polynomial(coeffs)
            ...
            if isa(coeffs,'polynomial')
                self.coeffs = coeffs.coeffs;
                self.order  = coeffs.order;
                return;
            end
            
            % Otherwise, arg1 is coeffs vector
            lastnnz = find(coeffs ~= 0,1,'last');
            if isempty(lastnnz), lastnnz = 1; end;
            self.coeffs = coeffs(1:lastnnz);
        ...
        function  [poly1] = differentiate(self)
            
            if iszero(self), poly1=self; return; end;
            
            factors = 1:self.order;
            poly1   = polynomial(factors.*self.coeffs(2:end));
